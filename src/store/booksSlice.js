...
export const booksSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
    .addCase(fechBooks.pending, (state, action) => {
      state.status = 'loading';
    })
    .addCase(fechBooks.fulfilled, (state, action) => {
      state.status = 'succeeded';
      state.books = action.payload;
    })
    .addCase(fechBooks.rejected, (state, action) => {
      state.status = 'faild';
    ...
    .addCase(toggleRead.fulfilled, (state, action) => {
      state.books.map(book => { if (book.id == action.payload) { book.isRead = !book.isRead } })
    })
...
export const fechBooks = createAsyncThunk('books/fechBooks', async () => {
